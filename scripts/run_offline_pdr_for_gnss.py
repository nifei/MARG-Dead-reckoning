...
def export_pdr_v2_traj_csv(hf, t_sec_abs, output_path):
    ...
    # Optional diagnostic fields injected by GNSS correction modules.
    for col in [
        "step_scale_factor",
        "step_scale_obs",
        "step_scale_update_flag",
        "phase_state",
        "anchor_scale_factor",
        "anchor_yaw_offset_deg",
        "anchor_update_flag",
        "anchor_obs_scale",
        "anchor_obs_yaw_deg",
        "anchor_rmse_m",
...
def fit_similarity_2d(local_xy: np.ndarray, world_xy: np.ndarray) -> tuple[float, float, np.ndarray, float]:
    """
    Fit y ~= s * R(theta) * x + t in 2D by Umeyama style least squares.
    Returns (scale, theta_rad, translation_xy, rmse_m).
    """
    if local_xy.shape[0] < 3 or world_xy.shape[0] < 3:
        raise ValueError("too few points for similarity fit")
    x_mu = np.mean(local_xy, axis=0)
    y_mu = np.mean(world_xy, axis=0)
    x_c = local_xy - x_mu
    y_c = world_xy - y_mu
    var_x = float(np.mean(np.sum(x_c * x_c, axis=1)))
    if var_x < 1e-8:
        raise ValueError("degenerate local points")
    sigma = (y_c.T @ x_c) / float(local_xy.shape[0])
    u, d, vt = np.linalg.svd(sigma)
    sgn = np.sign(np.linalg.det(u @ vt))
    if sgn == 0:
        sgn = 1.0
    sdiag = np.diag([1.0, sgn])
    rot = u @ sdiag @ vt
    scale = float(np.trace(np.diag(d) @ sdiag) / var_x)
    theta = float(np.arctan2(rot[1, 0], rot[0, 0]))
    trans = y_mu - scale * (rot @ x_mu)
    y_hat = (scale * (rot @ local_xy.T)).T + trans
    rmse = float(np.sqrt(np.mean(np.sum((y_hat - world_xy) ** 2, axis=1))))
    return scale, theta, trans.astype(float), rmse
...
def apply_gnss_local_world_anchor_correction(
    hf: pd.DataFrame,
    ...
    vel_local = np.column_stack([vel_ned[:, 1], vel_ned[:, 0]])  # ENU-x/y in local frame
    vel_z = -vel_ned[:, 2].copy()
    pos_local = np.column_stack([pos_ned[:, 1], pos_ned[:, 0]])  # ENU-x/y in local frame
    ...
    for i in range(n):
        j = int(np.searchsorted(ref_t, float(t_sec_abs[i]), side="left"))
        j = max(0, min(len(ref_t) - 1, j))
        p = int(ref_phase[j])
        phase_ts[i] = 0 if p == 0 else (1 if p == 1 else 2)

    # Initialize from first GNSS sample if possible.
    g0_e = float(np.interp(float(t_sec_abs[0]), g_t, g_e))
    g0_n = float(np.interp(float(t_sec_abs[0]), g_t, g_n))
    s_cur = 1.0
    th_cur = 0.0
    r_cur = rotation_2d(th_cur)
    t_cur = np.array([g0_e, g0_n], dtype=float) - s_cur * (r_cur @ pos_local[0])
    world_pos = np.zeros((n, 2), dtype=float)
    world_vel = np.zeros((n, 2), dtype=float)
    world_pos[0] = s_cur * (r_cur @ pos_local[0]) + t_cur

    scale_ts = np.full(n, s_cur, dtype=float)
    yaw_ts = np.full(n, np.degrees(th_cur), dtype=float)
    update_flag = np.zeros(n, dtype=int)
    obs_scale_ts = np.full(n, np.nan, dtype=float)
    obs_yaw_ts = np.full(n, np.nan, dtype=float)
    obs_rmse_ts = np.full(n, np.nan, dtype=float)

    half_w = float(max(window_sec, 1e-3) * 0.5)
    fit_count = 0
    update_count = 0

    for i in range(1, n):
        # Predict using current (s, theta).
        r_cur = rotation_2d(th_cur)
        v_world = s_cur * (r_cur @ vel_local[i])
        world_vel[i] = v_world
        world_pos_pred = world_pos[i - 1] + v_world * dt[i]
        t_pred = world_pos_pred - s_cur * (r_cur @ pos_local[i])
        # TODO 平移传播取反
        # t_pred = world_pos_pred - s_cur * (r_cur @ (-1) * pos_local[i])

        # Build local-world correspondences inside sliding window.
        ti = float(t_sec_abs[i])
        mask = (t_sec_abs >= ti - half_w) & (t_sec_abs <= ti + half_w)
        idx = np.where(mask)[0]
        has_obs = False
        s_obs = np.nan
        th_obs = np.nan
        t_obs = np.array([np.nan, np.nan], dtype=float)
        rmse_obs = np.nan
        if idx.size >= int(max(min_points, 3)):
            xw = pos_local[idx]
            yw = np.column_stack(
                [
                    np.interp(t_sec_abs[idx], g_t, g_e),
                    np.interp(t_sec_abs[idx], g_t, g_n),
                ]
            )
            # Degenerate window guard.
            if float(np.std(xw[:, 0]) + np.std(xw[:, 1])) > 1e-3:
                try:
                    # TODO: 计算变换入口取反
                    # xw_reverse = -1.0 * xw
                    s_obs, th_obs, t_obs, rmse_obs = fit_similarity_2d(xw, yw)
                    # s_obs, th_obs, t_obs, rmse_obs = fit_similarity_2d(xw_reverse, yw)

                    has_obs = np.isfinite(s_obs) and np.isfinite(th_obs) and np.isfinite(rmse_obs)
                except Exception:
                    has_obs = False

        alpha = 0.0
        if i > 0 and phase_ts[i] != phase_ts[i - 1]:
            pass # print(f"[anchor][change] to phase {phase_ts[i]} t_sec_abs={ti:.3f}, s_cur={s_cur:.6f}")
        if phase_ts[i] == 0:
            alpha = float(alpha_good)
        elif phase_ts[i] == 1:
            alpha = float(alpha_weak)
        else:
            alpha = 0.0
            # print(f"[anchor][Denied] t_sec_abs={ti:.3f}, s_cur={s_cur:.6f}")

        use_obs = (
            has_obs
            and alpha > 0.0
            and rmse_obs <= float(max_rmse_m)
            and np.isfinite(s_obs)
            and (float(r_min) <= float(s_obs) <= float(r_max))
        )

        if use_obs:
            # TODO 变换入口取反
            fit_count += 1
            s_new = (1.0 - alpha) * s_cur + alpha * float(np.clip(s_obs, r_min, r_max))
            dth = wrap_rad(float(th_obs) - th_cur)
            th_new = wrap_rad(th_cur + alpha * dth)
            t_new = (1.0 - alpha) * t_pred + alpha * t_obs
            s_cur = float(s_new)
            th_cur = float(th_new)
            t_cur = t_new.astype(float)
            update_flag[i] = 1
            update_count += 1
            obs_scale_ts[i] = float(s_obs)
            obs_yaw_ts[i] = float(np.degrees(th_obs))
            obs_rmse_ts[i] = float(rmse_obs)
        else:
            # Denied or no valid observation: freeze s/theta, only propagate t.
            t_cur = t_pred.astype(float)

        r_cur = rotation_2d(th_cur)
        world_pos[i] = s_cur * (r_cur @ pos_local[i]) + t_cur
        scale_ts[i] = s_cur
        yaw_ts[i] = float(np.degrees(th_cur))

    pos_x_scaled = world_pos[:, 0]
    pos_y_scaled = world_pos[:, 1]
    pos_z_scaled = np.zeros(n, dtype=float)
    for i in range(1, n):
        pos_z_scaled[i] = pos_z_scaled[i - 1] + vel_z[i] * dt[i]

    # Recompute velocity from world positions to keep kinematic consistency.
    vel_x_scaled = np.zeros(n, dtype=float)
    vel_y_scaled = np.zeros(n, dtype=float)
    valid_dt = dt > 0.0
    vel_x_scaled[valid_dt] = np.diff(pos_x_scaled, prepend=pos_x_scaled[0])[valid_dt] / dt[valid_dt]
    vel_y_scaled[valid_dt] = np.diff(pos_y_scaled, prepend=pos_y_scaled[0])[valid_dt] / dt[valid_dt]
    ...
    hf["anchor_scale_factor"] = scale_ts
    hf["anchor_yaw_offset_deg"] = yaw_ts
    hf["anchor_update_flag"] = update_flag
    hf["anchor_obs_scale"] = obs_scale_ts
    hf["anchor_obs_yaw_deg"] = obs_yaw_ts
    hf["anchor_rmse_m"] = obs_rmse_ts
    # Keep backward-compatible names for downstream scripts.
    hf["step_scale_factor"] = scale_ts
    hf["step_scale_obs"] = obs_scale_ts
    ...
    report = {
        "enabled": True,
        "method": "local_world_anchor_similarity",
        "satellite_csv": str(satellite_csv),
        "phase_ref_csv": str(phase_ref_csv) if phase_ref_csv else "",
        "window_sec": float(window_sec),
        "min_points": int(min_points),
        "max_rmse_m": float(max_rmse_m),
        "anchor_fit_count": int(fit_count),
        "anchor_update_count": int(update_count),
        ...
        "yaw_offset_mean_deg": float(np.nanmean(yaw_ts)),
        "yaw_offset_p95_deg": float(np.nanpercentile(np.abs(yaw_ts), 95)),
        "obs_rmse_mean_m": float(np.nanmean(obs_rmse_ts)) if np.isfinite(obs_rmse_ts).any() else float("nan"),
    }
    return hf, report
...
def main():
    ...
    parser.add_argument(
        ...
        default="anchor",
        choices=["anchor", "rk"],
        help="GNSS修正模式: anchor=局部轨迹锚定到世界坐标; rk=仅步长比例",
    )
    parser.add_argument(
        "--anchor-window-sec",
        type=float,
        default=10.0,
        help="anchor模式滑动窗口长度(秒)",
    )
    parser.add_argument(
        "--anchor-min-points",
        type=int,
    ...
    if sat_csv_arg:
        ...
        if sat_csv.exists():
            if args.gnss_corr_mode == "anchor":
                print("\n正在应用 GNSS 局部轨迹锚定修正 (p_world = s*R*p_local + t)...")
                hf_export, gnss_scale_report = apply_gnss_local_world_anchor_correction(
                    hf_export,
                    t_sec_abs=t_sec_abs,
                    ...
                    window_sec=float(args.anchor_window_sec),
                    min_points=int(args.anchor_min_points),
                    max_rmse_m=float(args.anchor_max_rmse_m),
                )
                print(
                    "GNSS锚定修正完成: updates="
                    f"{gnss_scale_report['anchor_update_count']}, "
                    f"mean_scale={gnss_scale_report['scale_mean']:.4f}"
                )
            else:
